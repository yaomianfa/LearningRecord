...
class utimeq:
    def __init__(self, max_queue_size: int) -> None:
        self.max_size = max_queue_size  # 队列最大容量
        self.heap = []  # 使用堆结构存储任务

    def push(self, time: int, callback: Any, value: Any) -> None:
        """插入任务到队列，若队列未满则按时间顺序插入"""
        if len(self.heap) < self.max_size:
            heapq.heappush(self.heap, (time, callback, value))

    def pop(self, entry: List[Any]) -> None:
        """将所有任务按时间顺序弹出到 entry 列表"""
    ...
    def peektime(self) -> int:
        ...
        if not self.heap:
            ...
            raise IndexError("peektime() called on empty queue")
    ...
    def discard(self, callback: Any) -> None:
        ...
        new_heap = []
        while self.heap:
            time, cb, val = heapq.heappop(self.heap)
            if cb != callback:  # 保留非目标回调的任务
                heapq.heappush(new_heap, (time, cb, val))
        self.heap = new_heap  # 替换为新堆
import time
class utime:
    def ticks_ms() -> int:
        t = time.time()
    ...
    def ticks_diff(a,b) -> int:
        return a-b;
    def ticks_add(a,b) -> int:
        return a+b
class io:
    def poll(a,b,c) -> bool:
        print("test poll",c)
        deadline = utime.ticks_ms() + c
...
class Syscall:
    ...
    def handle(self, task: Task) -> None:
        pass
    def __await__(self):
        return self.__iter__()  # Python 3.5+ 需要这个
...
class sleep(Syscall):
    ...
    """
    #print(f"missed by {utime.ticks_ms()} ms")
    

    def __init__(self, delay_ms: int) -> None:
     #   print(f"missed by {utime.ticks_ms()} {delay_ms} ms")
...
class chan:
     ...
     def put(self, value: Any) -> Awaitable[None]:  # type: ignore [awaitable-return-type]
         ...
         try:
             return  put  # type: ignore [awaitable-return-type]
         except:  # noqa: E722
     ...
     def take(self) -> Awaitable[Any]:  # type: ignore [awaitable-return-type]
         take = chan.Take(self)
         try:
             return  take  # type: ignore [awaitable-return-type]
         except:  # noqa: E722
...
async def test_task2():
    ...
    while True:
        await sleep(1000)
        print("我是一秒定时器")
        await loopchan.put("put with comm")
        print("put complete")
async def test_task3():
    while True:
        result = await loopchan.take()
